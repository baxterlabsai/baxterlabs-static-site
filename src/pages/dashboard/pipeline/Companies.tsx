...
function QuickActions({ detail }: { detail: CompanyDetail }) {
  ...
  const actions = [
    {
      label: 'Research',
      text: `/baxterlabs-sales:sales-research ${detail.name}`,
      icon: 'M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z',
      color: 'border-gray-light hover:border-teal hover:bg-teal/5',
    },
    {
      label: 'Draft Outreach',
      text: contactName ? `/baxterlabs-sales:sales-outreach Draft outreach to ${contactName} at ${detail.name}` : `/baxterlabs-sales:sales-outreach Draft outreach to ${detail.name}`,
      icon: 'M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75',
      color: 'border-gray-light hover:border-teal hover:bg-teal/5',
    },
    {
      label: 'Call Prep',
      text: `/baxterlabs-sales:sales-call-prep Prep me for my call with ${detail.name}`,
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap gap-2">
        {/* 1. Research */}
        <button
          onClick={() => copyToClipboard(actions[0].text)}
          className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-medium text-charcoal transition-colors ${actions[0].color}`}
        >
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d={actions[0].icon} />
          </svg>
          {actions[0].label}
        </button>
        {/* 2. Enrich (with staleness indicator) */}
        <div className="relative group">
          <button
            onClick={() => copyToClipboard(`/baxterlabs-sales:sales-enrichment Enrich ${detail.name}`)}
        ...
        {/* 3. Draft Outreach */}
        <button
          onClick={() => copyToClipboard(actions[1].text)}
          className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-medium text-charcoal transition-colors ${actions[1].color}`}
        >
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d={actions[1].icon} />
          </svg>
          {actions[1].label}
        </button>
        {/* 4. Call Prep */}
        <button
          onClick={() => copyToClipboard(actions[2].text)}
          className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-medium text-charcoal transition-colors ${actions[2].color}`}
        >
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d={actions[2].icon} />
          </svg>
          {actions[2].label}
        </button>
        {isPartner && (
          <button
            onClick={() => copyToClipboard(
              contactName
                ? `/baxterlabs-sales:sales-outreach Draft partnership outreach to ${contactName} at ${detail.name}`
                : `/baxterlabs-sales:sales-outreach Draft partnership outreach to ${detail.name}`
            )}
        ...
        {activeTab === 'research' && (
          researchContent ? (
            <div>
              ...
              <div className={`bg-ivory/50 border border-gray-light rounded p-3 overflow-y-auto ${variant === 'popout' ? 'max-h-[calc(100vh-200px)]' : 'max-h-[500px]'}`}>
          ...
          enrichment ? (
            <div>
              ...
              {enrichment.data && typeof enrichment.data === 'object' && Object.keys(enrichment.data).length > 0 ? (
                <div className={`bg-ivory/50 border border-gray-light rounded overflow-hidden overflow-y-auto ${variant === 'popout' ? 'max-h-[calc(100vh-200px)]' : 'max-h-[500px]'}`}>
                    ...
                    {detail.opportunities.length === 0 ? (
                      ...
                      <div className="space-y-2">
                        {detail.opportunities.map(opp => (
                          <div key={opp.id} className="bg-ivory/50 rounded-lg px-3 py-2">
                            <div className="flex items-center justify-between">
                              <div className="flex-1 min-w-0">
                              ...
                              {editingCallNotesOppId === opp.id ? (
                                <div className="flex items-center gap-1.5">
                                  <input
                                    ...
                                    className="flex-1 text-xs border border-gray-light rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-teal"
                                    autoFocus
                                    onKeyDown={e => {
                                      if (e.key === 'Escape') { setEditingCallNotesOppId(null); setCallNotesUrl('') }
                                    }}
                                  />
                                  <button
                                    onClick={async () => {
                                      await apiPut(`/api/pipeline/opportunities/${opp.id}`, { call_notes_doc_url: callNotesUrl || null })
                                      setEditingCallNotesOppId(null)
                                      setCallNotesUrl('')
                                      apiGet<CompanyDetail>(`/api/pipeline/companies/${companyId}`).then(setDetail).catch(() => {})
                                    }}
                                    className="text-xs px-2 py-1 bg-teal text-white rounded hover:bg-teal/90"
                                  >Save</button>
                                  <button
                                    onClick={() => { setEditingCallNotesOppId(null); setCallNotesUrl('') }}
                                    className="text-xs px-2 py-1 text-gray-warm hover:text-charcoal"
                                  >&times;</button>
                                </div>
                              ) : opp.call_notes_doc_url ? (
                                <div className="flex items-center gap-1.5">
                                  <a
                                    href={opp.call_notes_doc_url}
                                    target="_blank"
                                  ...
                                  <button
                                    onClick={() => { setEditingCallNotesOppId(opp.id); setCallNotesUrl(opp.call_notes_doc_url || '') }}
                                    className="text-gray-warm hover:text-charcoal"
                                  >
                                ...
                                <button
                                  onClick={() => { setEditingCallNotesOppId(opp.id); setCallNotesUrl('') }}
                                  className="text-xs text-gray-warm hover:text-teal inline-flex items-center gap-1"
                                >
                                  <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
                                  Add Call Notes
                                </button>
                              )}
                            </div>
