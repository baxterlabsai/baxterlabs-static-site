...
function QuickActions({ detail }: { detail: CompanyDetail }) {
  ...
  const actions = [
    {
      label: 'Research',
      text: `/sales-research ${detail.name}`,
      icon: 'M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z',
      color: 'border-gray-light hover:border-teal hover:bg-teal/5',
    },
    {
      label: 'Draft Outreach',
      text: contactName ? `/sales-outreach Draft outreach to ${contactName} at ${detail.name}` : `/sales-outreach Draft outreach to ${detail.name}`,
      icon: 'M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75',
      color: 'border-gray-light hover:border-teal hover:bg-teal/5',
    },
    {
      label: 'Call Prep',
      text: `/sales-call-prep Prep me for my call with ${detail.name}`,
    ...
    <div className="space-y-2">
      <div className="flex flex-wrap gap-2">
        ...
        <div className="relative group">
          <button
            onClick={() => copyToClipboard(`/sales-enrichment Enrich ${detail.name}`)}
        ...
        {isPartner && (
          <button
            onClick={() => copyToClipboard(
              contactName
                ? `/sales-outreach Draft partnership outreach to ${contactName} at ${detail.name}`
                : `/sales-outreach Draft partnership outreach to ${detail.name}`
            )}
