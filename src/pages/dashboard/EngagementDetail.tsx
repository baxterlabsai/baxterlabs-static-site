...
export default function EngagementDetail() {
  ...
  const [deliverablesApproved, setDeliverablesApproved] = useState(false)
  ...
  return (
    <div className="max-w-5xl">
      ...
      {!isClosed && data && (() => {
        ...
        // Send Deliverables requires explicit approval via the ribbon "Approve All" button
        const canSend = (deliverablesApproved || alreadySent) && pdfsDone
        const stepDone = [
        ...
        return (
          <section className="bg-white rounded-lg border border-gray-light p-5 mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {/* Send Deliverables (index 9) — available after approval in ribbon */}
              <button
                disabled={sendingDeliverables || !canSend}
                onClick={async () => {
                  if (alreadySent || !canSend) return
                ...
                title={alreadySent ? `Sent to ${data.deliverables_sent_to}` : canSend ? 'Send deliverables email to client' : 'Approve deliverables in ribbon first'}
              >
        ...
        return (
          <section className="bg-white rounded-lg border border-gray-light p-5 mb-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs text-gray-warm ml-auto mr-3">{pdfFiles.length} PDF{pdfFiles.length !== 1 ? 's' : ''} in Drive</span>
              {!deliverablesApproved && !data.deliverables_sent_at ? (
                <button
                  onClick={() => {
                    setDeliverablesApproved(true)
                    toast('All deliverables approved — Send Deliverables is now active', 'success')
                  }}
                ...
                <span className="text-xs font-semibold text-green bg-green/10 px-2.5 py-1 rounded-full">Approved</span>
              )}
            </div>
            <div className="space-y-2">
