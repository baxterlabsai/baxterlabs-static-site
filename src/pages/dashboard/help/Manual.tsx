// OPERATIONS MANUAL IN-DASHBOARD VIEWER
//
// This page renders the Operations Manual markdown from Supabase Storage
// via GET /api/help/operations-manual. It is read-only — the manual is a
// single file at operations-manual/current.md in the "manuals" bucket.
// There is no database table, no CRUD. When a new version is uploaded to
// storage, the dashboard picks up the new content on next page load.
//
// Do not introduce a database table for the manual content.
...
function parseToc(markdown: string): TocPart[] {
  const parts: TocPart[] = []
  ...
  for (const line of lines) {
    ...
    if (partMatch) {
      parts.push({ title: partMatch[1], slug: slugify(partMatch[1]), chapters: [] })
      continue
    }
    const chapterMatch = line.match(/^## (Chapter .+)/)
    if (chapterMatch && parts.length > 0) {
      parts[parts.length - 1].chapters.push({
        title: chapterMatch[1],
        slug: slugify(chapterMatch[1]),
      })
    }
  }
...
function chaptersWithMatches(markdown: string, query: string): Set<string> {
  ...
  for (const line of lines) {
    const partMatch = line.match(/^# (Part .+)/)
    if (partMatch) {
      currentSlug = slugify(partMatch[1])
    ...
    const chapterMatch = line.match(/^## (Chapter .+)/)
    if (chapterMatch) {
      currentSlug = slugify(chapterMatch[1])
    }
...
export default function Manual() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    apiGet<ManualResponse>('/api/help/operations-manual')
      .then((res) => {
        if (!cancelled) {
          setData(res)
          setLoading(false)
      ...
      .catch((err) => {
        if (!cancelled) {
          setError(err.message || 'Failed to load the Operations Manual.')
          setLoading(false)
        }
      })
  ...
  const matchingChapters = useMemo(
    () => (data ? chaptersWithMatches(data.content, debouncedQuery) : new Set<string>()),
    [data, debouncedQuery]
  )
  ...
  const handleDownload = () => {
    ...
    a.href = url
    a.download = `BaxterLabs_Operations_Manual_${data.version}.md`
  ...
  if (loading) {
    return (
      <>
        <SEO title="Operations Manual — BaxterLabs" description="BaxterLabs operations manual" />
            ...
            <div className="w-72 border-r border-charcoal/10 p-4 space-y-3 animate-pulse hidden lg:block">
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="space-y-2">
                  ...
                  <div className="h-3 bg-charcoal/5 rounded ml-3" style={{ width: `${50 + (i % 4) * 10}%` }} />
                  <div className="h-3 bg-charcoal/5 rounded ml-3" style={{ width: `${40 + (i % 5) * 10}%` }} />
                </div>
              ))}
            </div>
  ...
  if (!data || !data.content) {
    return (
      <>
        <SEO title="Operations Manual — BaxterLabs" description="BaxterLabs operations manual" />
        <div className="flex items-center justify-center h-full">
          <p className="text-charcoal/50">The Operations Manual is empty or could not be parsed.</p>
        </div>
      </>
    )
  }
    ...
    <>
      ...
      <div className="h-full flex flex-col manual-viewer">
        ...
        <div className="shrink-0 border-b border-charcoal/10 flex items-center gap-3 px-4 py-2.5 flex-wrap sm:flex-nowrap">
          ...
          <h1 className="text-lg font-display font-bold text-charcoal whitespace-nowrap">Operations Manual</h1>
        ...
        <div className="flex flex-1 min-h-0 relative">
          ...
          >
            ...
            <div className="lg:hidden flex items-center justify-between p-3 border-b border-charcoal/10">
              <span className="text-sm font-semibold text-charcoal">Contents</span>
            ...
            <div className="p-3 space-y-1">
              {toc.map((part) => {
                const partHasMatch = !debouncedQuery || matchingChapters.has(part.slug) || part.chapters.some((c) => matchingChapters.has(c.slug))
                return (
                  <div key={part.slug} style={{ opacity: debouncedQuery && !partHasMatch ? 0.4 : 1 }}>
                    <button
                      onClick={() => scrollToHeading(part.slug)}
                      className={`
                        w-full text-left text-xs font-bold text-charcoal/80 py-1.5 px-2 rounded
                        hover:bg-cream/60 transition-colors truncate
                        ${activeSlug === part.slug ? 'border-l-2 border-crimson bg-cream/40' : ''}
                      `}
                    >
                      {part.title}
                    </button>
                    <div className="ml-2 space-y-0.5">
                      {part.chapters.map((ch) => {
                        const chHasMatch = !debouncedQuery || matchingChapters.has(ch.slug)
                        return (
                          <button
                            key={ch.slug}
                            onClick={() => scrollToHeading(ch.slug)}
                            style={{ opacity: debouncedQuery && !chHasMatch ? 0.4 : 1 }}
                            className={`
                              w-full text-left text-[11px] leading-snug text-charcoal/65 py-1 px-2 rounded
                              hover:bg-cream/60 transition-colors line-clamp-2
                              ${activeSlug === ch.slug ? 'border-l-2 border-crimson bg-cream/40 text-charcoal/90 font-medium' : ''}
                            `}
                          >
                            {ch.title}
                          </button>
...
function SearchHighlightedMarkdown({ content, query }: { content: string; query: string }) {
  ...
  // Apply search highlighting via DOM after render
  useEffect(() => {
